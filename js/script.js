...
document.addEventListener('DOMContentLoaded', function () {
    ...
    timelineData.forEach(item => {
        ...
        div.innerHTML = `
            <div class="flex gap-6 items-start pl-2">
                ...
                <!-- IMAGE BOX -->
                <div class="${item.imageSize} ${item.imageMargin} rounded-lg overflow-hidden shadow-md">
                    <img src="${item.image}"
                        alt="${item.title}"
                ...
                ${item.source ? `
                    <a href="${item.source.link}"
                        target="_blank"
                        class="block text-[11px] text-white/70 italic mt-1">
                        ${item.source.text}
                    </a>
                ` : ''}
            </div>
            ...
            <div>
                ...
                <ul class="list-disc list-inside text-[#fcfafa] space-y-1">
                    ${pointsHTML}
                </ul>
            </div>
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                ...
                y: {
                    ticks: {
                        color: "#FFFFFF", // <<-- Microsoft, Python, etc labels color
                        padding: 20,
                        font: {
                            size: 14,
    ...
    window.addEventListener('scroll', () => {
        if (window.scrollY > 50) {
            ...
            header.querySelector('a[href="#hero"]').classList.add('text-[#004e96]'); // Main text color
            document.querySelectorAll('.nav-link').forEach(link => {
                link.classList.remove('text-white', 'hover:text-gray-200');
                link.classList.add('text-[#004e96]', 'hover:text-[#222222]'); // Secondary text color
            });
            ...
            header.querySelector('a[href="#hero"]').classList.remove('text-[#004e96]');
            header.querySelector('a[href="#hero"]').classList.add('text-white');
            document.querySelectorAll('.nav-link').forEach(link => {
                link.classList.remove('text-[#004e96]', 'hover:text-[#222222]');
